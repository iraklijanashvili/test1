...
// API გასაღებები
const WEATHER_API_KEY = '04c70df2f49a49abb73124624250904';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/news", async (req, res) => {
    try {
      // მომავალში აქ API გამოყენება იქნება, ახლა კი დროებით საგანგებოდ შექმნილ 
      // მონაცემებს ვიყენებთ, რადგან API არ გვაქვს.
      ...
      if (query) {
        ...
        sampleNews.articles = sampleNews.articles.filter(article => 
          article.title.toLowerCase().includes(lowercaseQuery) || 
          article.description.toLowerCase().includes(lowercaseQuery)
        );
        sampleNews.totalResults = sampleNews.articles.length;
      }
